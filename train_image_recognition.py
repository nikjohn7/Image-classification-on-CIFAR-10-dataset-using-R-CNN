...
# Building each neural network layer into a function.
# INPUT LAYER:

def neural_net_image_input(image_shape):
    return tf.compat.v1.placeholder(tf.float32, shape=(None, image_shape[0], image_shape[1], image_shape[2]), name='x')


def neural_net_label_input(n_classes):
    return tf.compat.v1.placeholder(tf.float32, shape=(None, n_classes), name='y')
...
def fully_conn(x_tensor, num_outputs):
    weight = tf.Variable(tf.random.truncated_normal(shape=[x_tensor.get_shape().as_list()[1], num_outputs], mean=0.0, stddev=0.1)) 
