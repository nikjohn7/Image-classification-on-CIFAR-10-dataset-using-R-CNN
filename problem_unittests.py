...
def test_con_pool(conv2d_maxpool):
    test_x = tf.compat.v1.placeholder(tf.float32, [None, 32, 32, 5])
    test_num_outputs = 10
    test_con_k = (2, 2)
    test_con_s = (4, 4)
    test_pool_k = (2, 2)
...
def test_flatten(flatten):
    test_x = tf.compat.v1.placeholder(tf.float32, [None, 10, 30, 6])
...
def test_fully_conn(fully_conn):
    test_x = tf.compat.v1.placeholder(tf.float32, [None, 128])
    test_num_outputs = 40
...
def test_conv_net(conv_net):
    test_x = tf.compat.v1.placeholder(tf.float32, [None, 32, 32, 3])
    test_k = tf.compat.v1.placeholder(tf.float32)
...
def test_train_nn(train_neural_network):
    mock_session = tf.compat.v1.Session()
